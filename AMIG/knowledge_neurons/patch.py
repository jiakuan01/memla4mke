...
def set_attribute_recursive(x: nn.Module, attributes: "str", new_attribute: nn.Module):
    ...
    for attr in attributes.split(".")[:-1]:
        x = getattr(x, attr)
    setattr(x, attributes.split(".")[-1], new_attribute)    # Set mlp.down_proj to mlp.Patch
...
def register_hook(
    ...
    def hook_fn(m, i, o):
        f(o)
    # for llama3
    # def hook_fn_llama(m, i, o):
    #     # print(f"type i = {type(i)}, len(i)= {len(i)}, shape: {i[0].shape}")
    #     f(i[0])
...
def patch_ff_layer(
    ...
    if mode == "replace":
        ff_layer = get_attributes(transformer_layers[layer_idx], ff_attrs)   # model.layers.x.mlp.down_proj
        assert layer_idx < len(
            transformer_layers
        ), f"cannot get layer {layer_idx + 1} of a {len(transformer_layers)} layer model"

        set_attribute_recursive(
            transformer_layers[layer_idx],  # model.layers.x
            ff_attrs,
            Patch(
                ff_layer,
                mask_idx,
